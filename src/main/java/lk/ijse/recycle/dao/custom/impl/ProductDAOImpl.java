...
public class ProductDAOImpl implements ProductDAO {
    ...
    public boolean update(Product dto) throws SQLException, ClassNotFoundException {
        return CrudUtil.execute(
                "UPDATE product SET product_name = ?, material_id = ?, kg_per_unit = ?, selling_price = ? WHERE product_id = ?",
                dto.getProduct_name(),
                dto.getMaterialId(),
                dto.getKgPerUnit(),
                dto.getSellingPrice(),
    ...
    public int getNextId() throws SQLException, ClassNotFoundException {
        ResultSet rs = CrudUtil.execute(
                "SELECT COALESCE(MAX(CAST(SUBSTRING(product_id, 2) AS UNSIGNED)), 0) + 1 AS next_id FROM product");
        if (rs.next()) {
    ...
    public boolean manufacture(String productId, int quantity) throws SQLException, ClassNotFoundException {
        ...
        try {
            ...
            ResultSet rs = CrudUtil.execute(
                    "SELECT item_id, item_kg FROM recycle_item WHERE material_id = ? AND item_kg > 0 ORDER BY date ASC FOR UPDATE",
